# 🪐 BiliUniverse: 🔀 Redirect
自动化重定向 CDN，让播放更流畅

## Loon 扩展

本 fork 额外提供一个仅面向 Loon 的可选 CDN 插件，用于将普通视频 `/upgcxcode/` 请求强制改写到所选节点，并提供基于真实视频分片吞吐率的自动测速模式。
需要 Loon `3.5.0(969)` 或更高版本。

- [一键导入 Loon](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Fvulpsecula%2FBiliBili-CDN-Loon%2Fmain%2Fcustom%2FBiliBili.Redirect.CCBStyle.plugin)
- [查看 Raw 插件](https://raw.githubusercontent.com/vulpsecula/BiliBili-CDN-Loon/main/custom/BiliBili.Redirect.CCBStyle.plugin)
- [使用说明](docs/CCB_STYLE_LOON.md)

插件节点列表按地区同步自 CCB。手动模式可直接选择具体 CDN；开启「⚡ 自动测速」后，会使用当前真实视频的签名分片 URL 分两阶段抽测候选节点，按实际有效吞吐率选择最快节点，并按网络缓存结果 6 小时。测速完成后会通知 Top 结果，也可从 `📊 查看 CDN 测速结果` Generic Script 查看缓存排名。

> [!IMPORTANT]
> 这是非官方 fork 的扩展，不隶属于 BiliUniverse、CCB、Bilibili Accelerator 或 Bilibili。不要与其他针对同一请求的固定 CDN 重写同时启用。
