...
插件菜单提供 `🎯 测试当前 CDN 持续带宽`。它不会重新选节点，而是针对当前正在使用/最近选择的单个 CDN 做更长、更稳定的串行测试：

- 自动模式优先取最近实际请求的 CDN；没有最近请求时取最新 family 缓存；手动模式直接测试当前手动节点；
- 使用固定公开测试视频动态获取新的 signed URL，不要求用户复制浏览器 Network 地址；
- 先做一次不计分预热和一次校准，再按校准结果自适应正式 Range 大小；
- 正式串行测试 3 轮，以中位数作为主要持续带宽结果，同时显示最低/最高值和稳定度；
- Wi-Fi 单轮最多 `8 MiB`，蜂窝网络最多 `4 MiB`，慢节点会自动缩小样本；
- 测试流量显式使用 `DIRECT`，且不会修改自动测速缓存、6 小时选择结果或当前 CDN。

这个手动测试参考 `scripts/bili_cdn_bandwidth.py` 的“预热不计分、串行多轮、中位数/最低值”思路，但受 Loon `$httpClient` 非流式回调限制，采用自适应 Range 大小代替桌面脚本的按时间窗口持续读取。
