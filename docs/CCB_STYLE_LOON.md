...
这是 `BiliUniverse/Redirect` 的非官方 Loon 扩展。插件保留完整 CCB CDN 节点的手动选择，同时提供基于真实视频分片吞吐率的自动测速。
...
不再使用早期的 `128 KiB 初筛 → 淘汰 → 512 KiB 精测` 逻辑，因此一个节点不会仅因为短初筛瞬时抖动就直接失去参与最终排名的资格。

### 3. 瞬时失败重试

首测出现 DNS、timeout 或其他连接类异常时，节点最多再重试一次：
...
HTTP 明确拒绝等非瞬时错误不会无意义反复重试。

### 4. Top 2 串行确认

如果首测/重试完成后至少有两个可用节点，而且 16 秒总预算还剩至少约 7 秒，会对 Top 2 再做一次串行确认：

- 每个节点 `512 KiB`；
- 每个节点最长约 `3.2 s`；
- 严格单并发，避免两个候选同时抢同一网络带宽。

只有 Top 2 **两个都确认成功**时，才使用这次串行结果重排前两名。如果只有一边成功，会保留之前的全量首测/重试排名，避免把不同测试条件的结果硬比较。

### 5. 总预算

一次无缓存测速有约 `16 s` 总预算。剩余时间不足时会跳过 Top 2 确认，而不是把 request script 拖到 Loon 的 30 秒脚本上限。
...
插件的手动下拉保留完整 CCB 地区节点。自动测速不会把数百个手动节点全部逐个测试，只使用代码中明确的小 family 候选池。

一些 gotcha / 302 / PCDN 风格节点仍可保留在手动列表中，但不会因此自动进入当前 family 测速池。
